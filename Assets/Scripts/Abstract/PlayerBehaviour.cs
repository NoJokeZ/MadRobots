...
public abstract class PlayerBehaviour : MonoBehaviour
{
    ...
    private bool gotDamageOnce = false;
    ...
    protected int specialAbilityAmmoCount;
    ...
    {
        //Sync values with camera scripts
    ...
    protected void FPVerticalMovement()
    {
        ...
        //Check if grounded
        isGrounded = Physics.Raycast(transform.position, Vector3.down, groundCheckRange, ~LayerMask.GetMask("Player"));
        ...
        if (jetPack.IsPressed() && JetPackDuration > 0)
        {
            ...
            JetPackDuration -= Time.deltaTime;
            if (JetPackDuration < 0) JetPackDuration = 0; //no values underneath 0 for fuel display
    ...
    {
        if (!gotDamageOnce) gotDamageOnce = true;
    ...
    {
        if (gotDamageOnce && HealthPoints <= 0)
        {
            gameManager.GameEnd();
            Destroy(gameObject);
        }
    ...
    /// <summary>
    /// Destroy player if they fall out of the map
    /// </summary>
